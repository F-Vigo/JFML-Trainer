...
import jfmltrainer.task.rulebasetrainer.tuning.lateral.selectionvariant.LateralSelectionVariant;
...
public abstract class LateralDisplacement {

    protected GrayCoder grayCoder;
    ...
    private ImmutablePair<List<Chromosome>, Boolean> nextGenerationAndNewIndividuals(
        ...
        List<Chromosome> shuffledPopulation = JFMLRandom.getInstance().shuffle(population);
    ...
    private List<ImmutablePair<Chromosome, Chromosome>> selectParentCandidates(List<Chromosome> population) {
        ...
        List<Integer> shuffledPositions = JFMLRandom.getInstance().shuffle(positions);
    ...
    private Float getRandom() {
        return JFMLRandom.getInstance().randReal(-0.5F, 0.5F);
    }
